...
class ListGrocerys extends State<ListGrocery> {
  ...
  List<Measure> usersEng = <Measure>[
    const Measure('Kg'),
    const Measure('Bag'),
    const Measure('Peice'),
    const Measure("Ryail")
  ];
  List<Measure> usersArb = <Measure>[
    const Measure('ك.ج'),
    const Measure('كيس'),
    const Measure("قطعه"),
    const Measure("ريال ")
        ...
        children: <Widget>[
          ...
          new Container(
              ...
              child: new Column(children: <Widget>[
                new ListTile(
                  title:
                      new Text(Intl.defaultLocale == 'ar_SA' ? "حول" : 'About'),
                  onTap: () {
                    showDialog(
                        context: context,
                        child: new SimpleDialog(
                          title: new Text(Intl.defaultLocale == 'ar_SA'
                              ? "هذا التطبيق في مرحلة تجريبية إذا كانت لديك أي فكرة لتحسين التطبيق أرسله إلينا نحن نقدر دعمك :D "
                          ...
                          children: <Widget>[
                            new RaisedButton(
                              ...
                              child: new Text(Intl.defaultLocale == 'ar_SA'
                                  ? "أوكي"
                                  : "OKAY"),
                            ),
                          ],
                ...
                new ListTile(
                  title: new Text(
                      Intl.defaultLocale == 'ar_SA' ? 'حفظ الملف' : 'Share'),
                  onTap: () {
                    ...
                    showDialog(
                        ...
                        child: new SimpleDialog(
                            title: new Text(Intl.defaultLocale == 'ar_SA'
                                ? "تم حفظ الملف في ملف التحميل. اذهب هنا وارسله"
                                : "File saved in Download file. \n Go there and "
                                "send it")));
                  },
                ),
                new Divider(),
      ...
      body: new Column(
        children: <Widget>[
          new TextField(
            ...
            decoration: Intl.defaultLocale == 'ar_SA'
                ? const InputDecoration(hintText: "أدخل الاغراض")
                : const InputDecoration(hintText: "Enter the Items "),
          ),
          new Row(children: <Widget>[
            ...
            new DropdownButton(
              hint: new Text(
                  Intl.defaultLocale == 'ar_SA' ? "المقياس" : "Measure!"),
              iconSize: 30.0,
              value: selectedUser,
              items: Intl.defaultLocale == 'ar_SA'
                  ? usersArb.map((Measure user) {
                      // arabic list of measure
                  ...
                  : usersEng.map((Measure user) {
                      //engligh list of measure
                      return new DropdownMenuItem(
                          value: user,
                          child: new Text(
                            user.name,
          ...
          new RaisedButton(
            onPressed: () {
              if (_controller.text == "" ||
                    ...
                    child: new SimpleDialog(
                      ...
                      children: <Widget>[
                        new RaisedButton(
                          ...
                          child: new Text(
                              Intl.defaultLocale == 'ar_SA' ? "أوكي" : "OKAY"),
                        ),
                      ],
                    ));
              } else if (_savedencode == null) {
                showDialog(
                    context: context,
                    child: new SimpleDialog(
                      title: new Text(Intl.defaultLocale == 'ar_SA'
                          ? " هل انت متاكد من عدم رغبتك لي اضافه صوره؟"
                          : "Are you sure you don't want add an Image"),
                      children: <Widget>[
                        new RaisedButton(
                          onPressed: () {
                            getImage();
                        ...
                        new RaisedButton(
                          onPressed: () {
                            Navigator.pop(context);
                            information(_controller.text, quantity,
                                selectedUser.name, _savedencode);
                          },
                          child: new Text(Intl.defaultLocale == 'ar_SA'
                              ? "نعم, بدون صوره"
                              : "Yes, No Image"),
                ...
                information(_controller.text, quantity, selectedUser.name,
                    _savedencode);
            },
          ...
          new Expanded(
            ...
            shrinkWrap: true,
            itemCount:
                TodoProvider.list.length == null ? 0 : TodoProvider.list.length,
              ...
              return new ListTile(
                title: new Text(
                    ...
                    style: (new TextStyle(
                        ...
                        color: Colors.black.withOpacity(1.0),
                        wordSpacing: 5.0))),
                trailing: TodoProvider.list[index]['picture'] == "null"
                    ? new Text(
                        TodoProvider.list[index]['quantity'].toString() +
                            " " +
                            TodoProvider.list[index]['type'],
                        style: (new TextStyle(
                            fontSize: 20.0,
                            fontWeight: FontWeight.bold,
                            color: Colors.black.withOpacity(1.0),
                ...
                onLongPress: () {
                  ...
                  showDialog(
                      ...
                      child: new SimpleDialog(
                        title: new Text(Intl.defaultLocale == 'ar_SA'
                            ? "!تم حذف الغرض "
                            : "Item have been deleted successfully"),
                        children: <Widget>[
                          new RaisedButton(
                            onPressed: () {
                              Navigator.pop(context);
  ...
  void importCallback() async {
    ...
    try {
      ...
      Navigator.of(context).push(new MaterialPageRoute(
          builder: (BuildContext context) => new OutsideList()));
    }
  ...
  imageshow() async {
    tpath = await getTemporaryDirectory();
  }
}
